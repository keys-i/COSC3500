...
class RendererContractTest(unittest.TestCase):
    ...
    def test_scene_metadata_accepts_extended_visual_contract(self):
        _, _, conway = visualise.load_scene_meta(
            ROOT / "proj/scenarios/conway/scene.meta"
        )
        self.assertEqual(conway["terminal_hold_seconds"], 0)

    def test_scene_metadata_preserves_unavailable_art(self):
        scene, _, visual = visualise.load_scene_meta(
            ROOT / "proj/scenarios/heston/scene.meta"
        )
        self.assertEqual(scene["art"], "unavailable")
        self.assertEqual(visual["camera"], "not renderable")
