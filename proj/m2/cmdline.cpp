...
class PageProbe final {
  public:
    explicit PageProbe(State &state) {
        ...
        const char *requested = std::getenv("M2_PAGE_POLICY");
...
int stream(Scenario scenario) {
    ...
    if (!scenario.lua_rules.empty() &&
        ...
        const std::string message = "m2: " + error + '\n';
...
int run(Scenario scenario, const std::string_view name, const bool snapshots,
...
#if defined(__linux__) && defined(PR_GET_THP_DISABLE) &&                       \
    defined(PR_SET_THP_DISABLE)
    const char *requested_page_policy = std::getenv("M2_PAGE_POLICY");
    ...
    if (!base_pages.valid()) {
        report_error("m2: cannot disable transparent huge pages\n");
        return 2;
    }
#endif
    State state = initialise(scenario);
    ...
    if (!base_pages.restore()) {
        report_error("m2: cannot restore transparent huge pages\n");
        return 2;
    }
#endif
    if (!page_policy.valid()) {
        report_error("m2: M2_PAGE_POLICY must be base or huge\n");
        return 2;
    }
    ...
    if (!scenario.lua_rules.empty() &&
        (!program.prepare(scenario, state, program_error) ||
         !setup_scenario_program(scenario, state, *program.get(),
                                 program_error))) {
        const std::string message = "m2: " + program_error + '\n';
        report_error(message.c_str());
        return 1;
    }
    ...
    if (snapshots) {
        if (!ensure_directory("results") ||
            ...
            report_error("m2: cannot create snapshot directory\n");
            return 1;
        }
        ...
        if (metrics.steps != scenario.world.steps && state.result < 0) {
            report_error("m2: simulation stopped before snapshot completion\n");
            return 1;
        }
        snapshot.publish();
    } else {
...
int run_cli(const int argc, char *argv[]) {
    try {
        constexpr char usage[] =
            "usage: m2 <templates|test>/<name> [--snapshots|--stream] "
            "[--seed auto|UINT64]\n"
            "       m2 --benchmark <templates|test>/<name> [--seed UINT64]\n";
        if (argc < 2) {
            report_error(usage);
            return 2;
        }
        ...
        report_error("m2: insufficient memory for scenario\n");
        return 1;
    } catch (const std::ios_base::failure &) {
        report_error("m2: snapshot write failed\n");
        return 1;
...
int main(const int argc, char *argv[]) { return m2::run_cli(argc, argv); }
