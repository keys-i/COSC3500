...
#ifndef M2_PHASE_PROFILE
#define M2_PHASE_PROFILE 0
#endif
...
[[nodiscard]] static Metrics simulate_continuous_impl(
    ...
    const bool use_verlet = pair_kernel && step_count > 1U && cutoff > 0.0 &&
                            (M2_WIDE_GRID != 0 || count <= 20'000'000U);
...
#endif
#if M2_OPT_LEVEL >= 6
    // Bound optional neighbour payload independently of population size
    // Allocator bookkeeping is reported separately through pair_list_bytes
    ...
    // Accepted edges replace the second grid traversal when they fit
    constexpr std::size_t csr_edge_scratch_cap = 8U * 1024U * 1024U;
    std::vector<std::uint64_t> csr_edges;
    if (csr_available) {
    ...
    const auto record_candidate_bytes = [&]() {
        metrics.pair_list_bytes = std::max<std::uint64_t>(
            ...
            pairs.capacity() * sizeof(std::uint64_t) +
                ...
                csr_edges.capacity() * sizeof(std::uint64_t));
    };
#endif
...
#if M2_PHASE_PROFILE && M2_OPT_LEVEL >= 7
    struct PhaseProfile {
        std::uint64_t fixed_member_steps = 0U;
        std::uint64_t prepare_ns = 0U;
        std::uint64_t csr_rebuild_steps = 0U;
        std::uint64_t csr_rebuild_ns = 0U;
        std::uint64_t csr_count_traversal_steps = 0U;
        std::uint64_t csr_count_traversal_ns = 0U;
        std::uint64_t csr_fill_traversal_steps = 0U;
        std::uint64_t csr_fill_traversal_ns = 0U;
        std::uint64_t csr_reuse_publish_steps = 0U;
        std::uint64_t csr_reuse_publish_ns = 0U;
        std::uint64_t other_search_steps = 0U;
        std::uint64_t other_search_ns = 0U;
        std::uint64_t commit_ns = 0U;
    } phase_profile;
    const auto add_profile_elapsed = [](std::uint64_t &total,
                                        const auto started) {
    ...
    const auto report_phase_profile = [&]() {
        const std::uint64_t csr_traversal_ns =
            phase_profile.csr_count_traversal_ns +
            phase_profile.csr_fill_traversal_ns;
        const std::uint64_t csr_rebuild_residual_ns =
            phase_profile.csr_rebuild_ns > csr_traversal_ns
                ? phase_profile.csr_rebuild_ns - csr_traversal_ns
                : 0U;
        std::fprintf(
            stderr,
            "M2_PHASE_PROFILE version=3 diagnostic=not_comparable "
            "fixed_member_steps=%llu prepare_ns=%llu "
            "csr_rebuild_steps=%llu csr_rebuild_ns=%llu "
            "csr_count_traversal_steps=%llu csr_count_traversal_ns=%llu "
            "csr_fill_traversal_steps=%llu csr_fill_traversal_ns=%llu "
            "csr_rebuild_residual_ns=%llu "
            "csr_reuse_publish_steps=%llu csr_reuse_publish_ns=%llu "
            "other_search_steps=%llu other_search_ns=%llu commit_steps=%llu "
            "commit_ns=%llu\n",
            static_cast<unsigned long long>(phase_profile.fixed_member_steps),
            static_cast<unsigned long long>(phase_profile.prepare_ns),
            static_cast<unsigned long long>(phase_profile.csr_rebuild_steps),
            static_cast<unsigned long long>(phase_profile.csr_rebuild_ns),
            static_cast<unsigned long long>(
                phase_profile.csr_count_traversal_steps),
            static_cast<unsigned long long>(
                phase_profile.csr_count_traversal_ns),
            static_cast<unsigned long long>(
                phase_profile.csr_fill_traversal_steps),
            static_cast<unsigned long long>(
                phase_profile.csr_fill_traversal_ns),
            static_cast<unsigned long long>(csr_rebuild_residual_ns),
            static_cast<unsigned long long>(
                phase_profile.csr_reuse_publish_steps),
            static_cast<unsigned long long>(
                phase_profile.csr_reuse_publish_ns),
            static_cast<unsigned long long>(phase_profile.other_search_steps),
            static_cast<unsigned long long>(phase_profile.other_search_ns),
            static_cast<unsigned long long>(phase_profile.fixed_member_steps),
            static_cast<unsigned long long>(phase_profile.commit_ns));
    };
#endif

    // Discover interactions before publishing the next state
    const auto publish = [&](Metrics &published_metrics,
                             [[maybe_unused]] double &published_travelled_squared,
                             const Scan &scan, const std::size_t entity,
        ...
        if (pair_list_available && !rebuild_pairs) {
            ...
            published_travelled_squared = std::max(
                published_travelled_squared,
...
#endif
#if M2_PHASE_PROFILE && M2_OPT_LEVEL >= 7
        const bool phase_profile_step = fixed_pair_membership;
#endif
#endif
#if M2_OPT_LEVEL < 2
        ...
        if (pair_kernel) {
...
#if M2_PHASE_PROFILE && M2_OPT_LEVEL >= 7
            if (phase_profile_step) {
                ++phase_profile.fixed_member_steps;
                add_profile_elapsed(phase_profile.prepare_ns, prepare_started);
            }
            const bool profile_csr_rebuild =
                pair_list_available && rebuild_pairs && csr_active;
            const bool profile_csr_reuse =
                pair_list_available && !rebuild_pairs && csr_active;
            ...
            if (pair_list_available && rebuild_pairs) {
                ...
                if (csr_active) {
                    ...
                    // The old adjacency is not needed during a rebuild
                    std::vector<GridIndex>{}.swap(adjacency);
                    std::vector<std::uint64_t>{}.swap(csr_edges);
                    const std::size_t adjacency_capacity =
                        (candidate_storage_cap - displacement_bytes -
                         csr_fixed_bytes) /
                        sizeof(GridIndex);
                    const std::size_t csr_edge_capacity = std::min(
                        csr_edge_scratch_cap / sizeof(std::uint64_t),
                        (candidate_storage_cap - displacement_bytes -
                         csr_fixed_bytes) /
                            (2U * sizeof(GridIndex) +
                             sizeof(std::uint64_t)));
                    if (csr_edge_capacity != 0U) {
                        csr_edges.reserve(csr_edge_capacity);
                        record_candidate_bytes();
                    }
                    std::size_t adjacency_count = 0U;
                    bool scratch_full = csr_edge_capacity == 0U;
...
#if M2_PHASE_PROFILE && M2_OPT_LEVEL >= 7
                    const auto csr_count_started =
                        profile_csr_rebuild ? std::chrono::steady_clock::now()
                    ...
                    traverse_cells([&](const std::size_t a, const std::size_t b,
                        ...
                        if (adjacency_count > adjacency_capacity ||
                            ...
                            ++adjacency_offsets[b + 1U];
                            adjacency_count += 2U;
                            if (csr_edges.size() == csr_edge_capacity) {
                                scratch_full = true;
                            } else {
                                csr_edges.push_back(
...
#if M2_PHASE_PROFILE && M2_OPT_LEVEL >= 7
                    if (profile_csr_rebuild) {
                        ++phase_profile.csr_count_traversal_steps;
                        add_profile_elapsed(phase_profile.csr_count_traversal_ns,
                                            csr_count_started);
                    }
#endif
                    if (!list_full) {
                        ...
                        // Preserve traversal order so equal-distance ties stay stable
                        if (!scratch_full) {
                            for (const std::uint64_t packed : csr_edges) {
                                const std::size_t a =
...
#pragma omp parallel
                            {
                                ...
                                double local_travelled_squared = 0.0;
#pragma omp for schedule(static)
                                for (std::size_t entity = 0U; entity < count;
                                     ++entity) {
                                    const Scan &scan =
                                        entity < scans[1].plan->first ? scans[0]
                                                                       : scans[1];
                                    ...
                                    if (certified && target != count) {
                                        best_x = wrapped_delta(
                                            state.x[target] - state.x[entity],
                                            scenario.world.width);
                                        best_y = wrapped_delta(
                                            state.y[target] - state.y[entity],
                                            scenario.world.height);
                                        best_squared = best_x * best_x +
                                                       best_y * best_y;
                                        ++local_metrics.candidate_checks;
                                        ++local_metrics.pair_evaluations;
                                        certified = best_squared <=
                                            scan.plan->sensing_radius_squared;
                                        local_metrics.sensed_interactions +=
                                    ...
                                    if (!certified) {
                                        ...
                                        for (std::size_t member =
                                            ...
                                            const std::size_t candidate =
                                                adjacency[member];
                                            const double delta_x = wrapped_delta(
                                                state.x[candidate] -
                                                    state.x[entity],
                                                scenario.world.width);
                                    ...
                                    publish(local_metrics,
                                            local_travelled_squared, scan,
                                            entity, target, best_squared,
                                            best_x, best_y);
                                }
#pragma omp critical
                                {
                                    ...
                                    next_travelled_squared = std::max(
                                        next_travelled_squared,
                                        local_travelled_squared);
                                }
                            }
                            return;
                        }
                        ...
                        for (std::size_t entity = 0; entity < count; ++entity) {
                            ...
                            publish(metrics, next_travelled_squared, scan,
                                    entity, target, best_squared, best_x,
                                    best_y);
                        }
                    };
                    if (fixed_pair_membership) {
            ...
            if (phase_profile_step) {
                if (profile_csr_rebuild) {
                    ++phase_profile.csr_rebuild_steps;
                    add_profile_elapsed(phase_profile.csr_rebuild_ns,
                                        search_started);
                } else if (profile_csr_reuse) {
                    ++phase_profile.csr_reuse_publish_steps;
                    add_profile_elapsed(phase_profile.csr_reuse_publish_ns,
                                        search_started);
                } else {
                    ++phase_profile.other_search_steps;
                    add_profile_elapsed(phase_profile.other_search_ns,
                                        search_started);
                }
            }
#endif
...
#if M2_PHASE_PROFILE && M2_OPT_LEVEL >= 7
        if (phase_profile_step) {
            add_profile_elapsed(phase_profile.commit_ns, commit_started);
        }
#endif
        ++metrics.steps;
...
[[nodiscard]] Metrics simulate_continuous(
    ...
    const StepController controller, void *const controller_context,
    const std::uint64_t first_step, const std::uint64_t step_count) {
#if M2_OPT_LEVEL == 1 && !M2_WIDE_GRID
