...
std::uint64_t checksum(const State &state) noexcept {
    ...
    for (std::size_t index = 0; index < state.x.size(); ++index) {
        ...
        if (index < state.timeline_rotation.size()) {
            hash_double(value, state.timeline_rotation[index]);
            hash_double(value, state.timeline_scale[index]);
            hash_double(value, state.timeline_opacity[index]);
        }
    }
...
std::uint64_t state_bytes(const State &state) noexcept {
    ...
    const std::size_t doubles =
        ...
        state.next_velocity_y.size() + state.timeline_rotation.size() +
        state.timeline_scale.size() + state.timeline_opacity.size() +
        state.timeline_start_rotation.size() + state.timeline_start_scale.size() +
        state.timeline_start_opacity.size() + state.timeline_target_rotation.size() +
        state.timeline_target_scale.size() + state.timeline_target_opacity.size() +
        state.pde.values.size() +
    ...
    const std::size_t wide =
        ...
        state.timeline_transform_start_step.size() +
        state.timeline_transform_end_step.size();
