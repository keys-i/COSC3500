...
[[nodiscard]] bool timeline_transform_ready(const State &state,
                                            const std::size_t size) noexcept {
    return state.timeline_rotation.size() == size &&
...
void initialise_timeline_transform(State &state, const std::size_t size) {
    if (!state.timeline_rotation.empty())
        return;
    state.timeline_rotation.assign(size, 0.0);
    state.timeline_scale.assign(size, 1.0);
    state.timeline_opacity.assign(size, 1.0);
    state.timeline_start_rotation.resize(size);
    state.timeline_start_scale.assign(size, 1.0);
    state.timeline_start_opacity.assign(size, 1.0);
    state.timeline_target_rotation.resize(size);
    state.timeline_target_scale.assign(size, 1.0);
    state.timeline_target_opacity.assign(size, 1.0);
    state.timeline_transform_start_step.resize(size);
    state.timeline_transform_end_step.resize(size);
}
    ...
    for (const ScriptCommand &command : commands) {
        ...
        if (command.kind == ScriptCommandKind::timed_move) {
            if (scenario.kernel != Kernel::timeline || command.duration == 0U ||
                command.duration > std::numeric_limits<std::uint64_t>::max() -
                                       step ||
                command.first < 0.0 || command.first >= scenario.world.width ||
                command.second < 0.0 ||
                command.second >= scenario.world.height ||
                (command.has_third &&
                 (!std::isfinite(command.third) || command.third < 0.0)) ||
                !std::isfinite(command.arc_height) || command.arc_height < 0.0) {
                return false;
            }
        ...
        } else if (command.kind == ScriptCommandKind::keyframe) {
            if (scenario.kernel != Kernel::timeline || command.duration == 0U ||
                command.duration > std::numeric_limits<std::uint64_t>::max() -
                                       step ||
                command.first < -36'000.0 || command.first > 36'000.0 ||
                command.second <= 0.0 || command.second > 100.0 ||
                command.third < 0.0 || command.third > 1.0) {
                return false;
            }
            initialise_timeline_transform(state, scenario.entity_count);
            if (!timeline_transform_ready(state, scenario.entity_count))
                return false;
            state.timeline_start_rotation[entity] = state.timeline_rotation[entity];
            state.timeline_start_scale[entity] = state.timeline_scale[entity];
            state.timeline_start_opacity[entity] = state.timeline_opacity[entity];
            state.timeline_target_rotation[entity] = command.first;
            state.timeline_target_scale[entity] = command.second;
            state.timeline_target_opacity[entity] = command.third;
            state.timeline_transform_start_step[entity] = step;
            state.timeline_transform_end_step[entity] = step + command.duration;
