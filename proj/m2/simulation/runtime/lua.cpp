...
[[nodiscard]] double non_negative(clx::LState *s, const clx::LValue &v,
                                  const char *const name) {
    const double n = finite(s, v);
    if (n < 0.0)
        fail(s, name);
    return n;
}
...
[[nodiscard]] clx::MultiValue engine_move(clx::LState *s, const clx::LValue *a,
    ...
    c.has_third = n == 4U;
    if (c.has_third)
        c.third = non_negative(s, a[3], "z must be non-negative");
    add(h, c);
    return {};
}
[[nodiscard]] clx::MultiValue
engine_timed_move(clx::LState *s, const clx::LValue *a, const std::size_t n) {
    ...
    c.has_third = n >= 5U;
    if (c.has_third)
        c.third = non_negative(s, a[4], "z must be non-negative");
    if (n == 6U)
        c.arc_height = non_negative(s, a[5], "arc must be non-negative");
    add(h, c);
    return {};
}
[[nodiscard]] clx::MultiValue
engine_keyframe(clx::LState *s, const clx::LValue *a, const std::size_t n) {
    if (n != 5U)
        fail(s, "keyframe needs entity, rotation, scale, opacity, duration");
    auto &h = host(s);
    ScriptCommand c = entity_command(s, h, ScriptCommandKind::keyframe, a[0]);
    c.first = finite(s, a[1]);
    c.second = finite(s, a[2]);
    c.third = finite(s, a[3]);
    c.duration = static_cast<std::uint64_t>(id(s, a[4]));
    if (c.duration == 0U)
        fail(s, "keyframe duration must be positive");
    add(h, c);
    return {};
}
[[nodiscard]] clx::MultiValue
...
[[nodiscard]] clx::LValue module(clx::LState *s) {
    ...
    clx::set_function(s, m, "keyframe", engine_keyframe);
